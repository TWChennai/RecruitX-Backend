defmodule RecruitxBackend.RoleSpec do
  ...
  let :valid_attrs, do: params_with_assocs(:role)
  ...
  context "is_ba_or_pm" do
    it "should return true if role is ba" do
      ...
      expect(Role.is_ba_or_pm(ba_role_id, Role.ba_and_pm_list)) |> to(be(true))
    end

    it "should return true if role is pm" do
      pm_role_id = Role.retrieve_by_name(Role.pm).id
      expect(Role.is_ba_or_pm(pm_role_id, Role.ba_and_pm_list)) |> to(be(true))
    end

    it "should return false if role is not pm or ba" do
      other_role_id = insert(:role).id
      expect(Role.is_ba_or_pm(other_role_id, Role.ba_and_pm_list)) |> to(be(false))
    end
  end
end
