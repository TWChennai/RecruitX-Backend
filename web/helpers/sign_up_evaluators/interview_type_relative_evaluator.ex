defmodule RecruitxBackend.InterviewTypeRelativeEvaluator do
  ...
  def evaluate(sign_up_evaluation_status, interview_type_specfic_criteria, panelist_login_name, panelist_role, interview, is_slot) do
    ...
    |> is_eligible_based_on_role(interview_type_specfic_criteria, panelist_role, interview, is_slot)
  end
  ...
  defp is_eligible_based_on_role(%{valid?: false} = sign_up_evaluation_status, _, _, _, _), do: sign_up_evaluation_status

  defp is_eligible_based_on_role(sign_up_evaluation_status, interview_type_specfic_criteria, panelist_role, interview, false) do
  ...
  defp is_eligible_based_on_role(sign_up_evaluation_status, interview_type_specfic_criteria, panelist_role, slot, true) do
    if !is_interview_type_with_specific_panelists(slot, interview_type_specfic_criteria)
    and (panelist_role |> is_nil or !(Role.is_ba_or_pm(panelist_role.id) and Role.is_ba_or_pm(slot.role_id)) and panelist_role.id != slot.role_id), do:
